...
| #25 | MCP server plugin | ✅ Implemented in `core/plugins/mcp/plugin.go`. Load with `proxyctl --plugins mcp_server` (standalone, blocks). Exposes `diagnose`, `list_checks`, and `compare` as MCP tools on `:9090`. Supports SSE transport (GET /mcp) and `initialize` handshake per MCP spec. Compatible with OpenCode via `opencode.jsonc`. |
